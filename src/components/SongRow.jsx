...
function SongRow({ s, i, track, playing, liked, setLiked, toggleFav, play, isDrive, isCached, onRemove, playlists, addToPlaylist, isLite, t, onDownload, editMode, embedTrack, isDownloading, dlProgress }) {
  ...
  return (
    <div data-songrow style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 12px', borderRadius:14, cursor:'pointer', background:isActive?s.bg:'rgba(255,255,255,0.04)', border:`1px solid ${isActive?s.color+'50':'transparent'}` }} onClick={()=>play(s)}>
      ...
      {isLite || isYtSong || s.isRadio
        ? <div style={{ width:42, height:42, borderRadius:10, background: s.isRadio ? `${s.color}20` : isYtSong ? 'rgba(255,68,68,0.15)' : (s.bg||'rgba(255,255,255,0.07)'), flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', position:'relative', overflow:'hidden' }}>
            {s.isRadio && !isLite && (s.cover||s.favicon)
              ? <img src={s.cover||s.favicon} alt={s.title} loading="lazy" decoding="async" style={{ width:'100%', height:'100%', objectFit:'cover', position:'absolute', inset:0 }} onError={e=>{ e.target.style.display='none'; }}/>
              : s.isRadio
              ? <Radio size={16} color={s.color}/>
              : isYtSong && (s.thumbnail||s.cover) && !isLite
            ...
            {s.isRadio && !isLite && (s.cover||s.favicon) && <div style={{ position:'absolute', bottom:2, right:2, fontSize:7, fontWeight:800, background:'rgba(245,158,11,0.85)', color:'white', padding:'1px 3px', borderRadius:3, lineHeight:1.2 }}>FM</div>}
          </div>
        : s.isDrive && s.cover
          ...
          : <div style={{ width:42, height:42, borderRadius:10, background:s.bg||'rgba(255,255,255,0.07)', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
              <Music size={16} color={s.color}/>
            </div>}
